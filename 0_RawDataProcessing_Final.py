### In this file, I am intending to get the patents as well as its CPCs from the PatentView.
### The output would be a database contained selected PatentID, as well as CPCs and subCPCs.

########## 0#####################

## Make sure you have 256G ram and 1024Gb diskspace
...
#########  3#######################
## Download file from a command line:
...
Desktop_Developing = True ## Step0,step2 has this function.


### Read Patent.tsv
patentRaw = "PatentsViewRaw/g_patent.tsv"

### If Desktop development, only choose the first 200,000 rows from patentView datasets.
### Otherwise I will select all qualified patents after 2010.
...
df_patentRaw = df_patentRaw.loc[df_patentRaw['patent_date'] <= '2022-12-31']
...
if Desktop_Developing:
    df_cpc_current = pd.read_csv(cpc_current_Raw, delimiter="\t", quoting = csv.QUOTE_NONNUMERIC, nrows=1000000)
else:
...
df_merged_cpc.to_csv('Results/0_df_V1.csv', index=False)
